...
passwd=$(cat /proc/sys/kernel/random/uuid  | md5sum |cut -c 1-16)
...
read -p "请输入从机密钥：" secret

echo "本程序默认安装在 $cloudreve_dir 目录"
mkdir $cloudreve_dir
...
cat<<EOF>$cloudreve_dir/conf.ini
[System]
Mode = slave
Listen = :5212
...
wget -O $aria2_conf_dir/aria2.tar.gz https://github.com/P3TERX/Aria2-Pro-Core/releases/download/${aria2_new_ver}/aria2-${aria2_new_ver%_*}-static-linux-amd64.tar.gz


tar -zxvf $aria2_conf_dir/aria2.tar.gz && rm -rf /root/.cloudreve/.aria2/aria2.tar.gz

mv -f aria2c $aria2_conf_dir/

chmod +x $aria2_conf_dir/aria2c
...
# 从会话文件中读取下载任务
input-file=/root/.cloudreve/.aria2/aria2.session

# 会话文件保存路径
# Aria2 退出时或指定的时间间隔会保存`错误/未完成`的下载任务到会话文件
save-session=/root/.cloudreve/.aria2/aria2.session
...
[Service]
Type=simple
ExecStart=${aria2_conf_dir}/aria2c --conf-path="${aria2_conf}"
Restart=on-failure
...
aria2_url=http://${IPV4}:${aria2_port}
cloudreve_url=http://${IPV4}:5212
...
echo -e "\nAria2 简单配置信息：\n
 ...
 RPC 地址\t: ${Green_font_prefix}${aria2_url}${Font_color_suffix}
 Cloudreve 地址\t: ${Green_font_prefix}${cloudreve_url}${Font_color_suffix}\n"